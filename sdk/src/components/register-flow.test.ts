...
function createMockSdk(overrides: Partial<SentinelAuth> = {}): SentinelAuth {
  return { register: vi.fn(), verifyEmail: vi.fn(), ...overrides } as unknown as SentinelAuth;
}
...
function submitRegister(el: any, email: string, password: string) {
  ...
  registerEl.shadowRoot.querySelector("form")
...
describe("sentinel-auth-register-flow", () => {
  ...
  it("shows register step by default", () => {
    ...
    expect(el.shadowRoot.querySelector(".register-step").classList.contains("active")).toBe(true);
    expect(el.shadowRoot.querySelector(".verify-step").classList.contains("active")).toBe(false);
  });
  ...
  it("switches to verify step on successful registration", async () => {
    ...
    expect(el.shadowRoot.querySelector(".verify-step").classList.contains("active")).toBe(true);
    expect(el.shadowRoot.querySelector(".register-step").classList.contains("active")).toBe(false);
  });
  ...
  it("displays the registered email in the verify step heading", async () => {
    ...
    expect(el.shadowRoot.querySelector(".verify-email").textContent).toBe("new@example.com");
  });
  ...
  it("dispatches sentinel-register-complete after successful verification", async () => {
    const sdk = createMockSdk({
      register: vi.fn().mockResolvedValue({ message: "check your email" }),
      verifyEmail: vi.fn().mockResolvedValue({ message: "Email verified successfully. You may now log in." }),
    });
    ...
    expect(completeHandler.mock.calls[0][0].detail.email).toBe("new@example.com");
  });

  it("shows error on OTP widget and stays on verify step when verifyEmail rejects", async () => {
    const sdk = createMockSdk({
      register: vi.fn().mockResolvedValue({ message: "check your email" }),
      verifyEmail: vi.fn().mockRejectedValue(new Error("Invalid verification code")),
    });
    ...
    const otpEl = el.shadowRoot.querySelector("sentinel-auth-otp");
    expect(otpEl.shadowRoot.querySelector(".error-message").textContent).toBe("Invalid verification code");
